    ...
    return {
        f'opensearch-{node_type}{node_id}': {
            ...
            'environment': [
                ...
                'discovery.seed_hosts=' + ','.join(
                    f'opensearch-{node_type}{i}' for i in
                    range(1, num_nodes + 1)),
                'cluster.initial_master_nodes=' + ','.join(
                    f'opensearch-{node_type}{i}' for i in
                    range(1, num_nodes + 1)),
            ...
            'ports': [
                f'{9200 + node_id - 1}:9200',
                f'{9600 + node_id - 1}:9600'
            ],
            'networks': [
                'opensearch-net'
            ]
...
for node_type in node_types:
    for i in range(1, num_nodes + 1):
        data['services'].update(create_node(i, node_type, opensearch_version,
                                            opensearch_java_opts))
...
data['services']['opensearch-dashboards'] = {
    ...
    'expose': ['5601'],
    'environment': {
        'OPENSEARCH_HOSTS': str(['https://' + f'opensearch-{node_type}{i}:9200'
                                 for node_type in node_types for i in
                                 range(1, num_nodes + 1)])
    },
