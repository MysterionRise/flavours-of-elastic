#!/usr/bin/env python3
"""
Generate abstract and description columns for movies.csv using OpenRouter LLM API.

Reads ml-32m/movies.csv (movieId, title, genres) and produces
data/movies_enriched.csv with two extra columns: abstract, description.
...
1. "abstract": 1-2 concise sentences describing what the movie is about.
2. "description": A detailed one-paragraph description (~100-150 words) covering \
...
Respond ONLY with valid JSON: {"abstract": "...", "description": "..."}
...
def write_output(path, rows):
    ...
    if not rows:
        return
    fieldnames = ["movieId", "title", "genres", "abstract", "description"]
...
def parse_llm_response(text):
    ...
    try:
        ...
        return data.get("abstract", ""), data.get("description", "")
        ...
        async with semaphore:
            try:
                async with session.post(
                    ...
                    abstract, description = parse_llm_response(content)
                    if abstract and description:
...
async def main():
    ...
    for i in range(0, total, args.batch_size):
        ...
        for movie, (abstract, description) in zip(batch, results):
            all_results[movie["movieId"]] = {
        ...
        failed = sum(1 for m in batch if not all_results[m["movieId"]].get("abstract"))
    ...
    elapsed = time.time() - start_time
    total_done = sum(1 for r in all_results.values() if r.get("abstract"))
