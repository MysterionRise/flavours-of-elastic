...
Our loader creates deterministic 384-dimensional embeddings for local practice:

```bash
python data/load_data.py --dataset movies --with-embeddings
```

This creates `movies-embeddings` with 100 movies, each containing:
...
| `year` | integer | Parsed release year |
| `release_date` | date | Derived release date |
| `overview_embedding` | dense_vector (384) | Deterministic local embedding |
...
{"index": {"_id": "2"}}
{"title": "The Godfather", "overview": "The aging patriarch of an organized crime dynasty transfers control of his clandestine empire to his reluctant son.", "overview_semantic": "The aging patriarch of an organized crime dynasty transfers control of his clandestine empire to his reluctant son.", "genres": ["Crime", "Drama"], "year": 1972}
```
